...
public class NullInputStreamTest {
    ...
    public void testRead() throws Exception {
        ...
        for (int i = 0; i < size - 1; i++) {
            input.read(); // Skip to the last byte
        }
        assertEquals("Check last byte value", size - 1, input.read());
        assertEquals("Available after reading last byte", 0, input.available());

        // Test reading one more byte after the last byte (boundary condition)
        assertEquals("End of File after reading last byte", -1, input.read());
        assertEquals("Available after reading last byte", 0, input.available());
    }
